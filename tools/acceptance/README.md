...
| 2026-09-24 | 同上，PHP 8.1.34 NTS DEBUG，gene 6.2.6 + swoole 6.1.9，MySQL + Redis + demo | **20/20 PASS** | `gene-swoole-result-20260924`；TestRunner **989/989**（`failed=0`，SKIP=7 均为环境缺服务：MySQL socket / PG / Redis AUTH，沿用「SKIP 视为通过」约定）；swoole-matrix digest `856ba31839fa8675`、entry digest `fd1425a4658c643a` 各自组内一致且与 6.2.5 逐位相同；entry-soak 10 万请求 19386 req/s、`co_contexts_items=0`、`ctx_pool` 未超限；两池 200×1000 `failures=0`/`commandFailures=0`、生命周期三场景全过（满池 waiter ~0.20s、close 交错 waiter 收 null、`total=0`）、`tx-hygiene` `POOL TX HYGIENE OK`；log-rotation 三探针全过（异常退出 100/100 行落盘）；demo wrk 2m **18708 req/s**（224.7 万请求，0 错误，p50 25.7ms/p99 54.8ms），worker RSS ~13.5MB 平稳（max_request 正常轮换）。`git_head=unavailable`，gene.so SHA-256 `4bb5825a…`。审计 T5 残项本轮仍未覆盖：PG `flip()` 继续 SKIP、协程桶隔离探针未实现 → R4 维持「待验证」，回填见 `audit/AUDIT_REPORT_2026_09_23.md` §十二。 |
