...
Class HandlerSocket
{
    ...
    {
        $this->_config = $config + array('host' => '127.0.0.1', 'port' => 9998, 'port_wr' => 9999, 'options' => array('timeout' => 2));
    ...
    public function select(array $query = array(), array $options = array())
    {
        ...
        $hsi = $this->connection()->open_index($dbname, $table, $options['fields'], array('index' => $index));
        $result = $hsi->find(array($query['op'] => $query['criteria']), array('limit' => $query['limit'], 'offset' => $query['skip']));
        if (!$result) {
    ...
    public function selectMulti($querys = array(), $options = array())
    {
        ...
        foreach ($querys as $key => $value) {
            $query[$key] = array('find', array($value['op'] => $value['criteria']), array('limit' => $value['limit'], 'offset' => $value['skip']));
        }
    ...
    public function update($query, $data, $options = array())
    {
        ...
        $hsi = $this->writeConnection()->open_index($dbname, $table, implode(',', array_keys($data)), array('index' => $index));
        $result = $hsi->update(array($query['op'] => $query['criteria']), array_values($data), array('limit' => $query['limit'], 'offset' => $query['skip']));
        if (!$result) {
            $this->_errorLog = $hsi->getError();
        }
        return $result;
    ...
    public function delete($query, $options = array())
    {
        ...
        $hsi = $this->writeConnection()->open_index($dbname, $table, $options['fields'], array('index' => $index));
        $result = $hsi->remove(array($query['op'] => $query['criteria']), array('limit' => $query['limit'], 'offset' => $query['skip']));
        if (!$result) {
            $this->_errorLog = $hsi->getError();
        }
        return $result;
