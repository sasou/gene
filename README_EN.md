...
> 💡 **Gene 6.2.3** — Unified FPM/Swoole entry runtime: `Request::initSwoole`, `Application::handleSwoole`, `bootstrap()` and explicit pool orchestration, new `gray` environment mapping, plus a `sendFile()` local-path regression fix.
