...
$list = $this->cache->cached(
    [\Services\User::getInstance(), 'getList'],
    [$page, $limit],
    300
);
...
| cachedVersion($obj, $args, $versionField, $ttl = null, $mode = null) | 带版本号控制的外部缓存 |
| localCachedVersion($obj, $args, $versionField, $ttl = null, $mode = null) | 带版本号控制的本地 APCu 缓存 |
| getVersion($version) | 从外部缓存读取指定版本字段的当前值 |
| updateVersion($version) | 对版本字段执行 incr，使关联缓存失效 |
