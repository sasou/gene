...
/* [GENE_PERF:2026-09-21 V3-3.2] Declared-property slot offsets, resolved once in
 ...
 * on every SQL builder call. */
#define GENE_DB_MYSQL_SET_PROP(KEY, ...) do { \
    zend_string *_gene_db_s_ = strpprintf(0, __VA_ARGS__); \
    gene_db_prop_set_str(gene_strip_obj(self), gene_db_mysql_prop[KEY], _gene_db_s_); \
    zend_string_release(_gene_db_s_); \
} while (0)

/* Benchmark variables removed from file scope to avoid coroutine data races.
 * They are now local to gene_mysql_pdo_execute and passed to mysqlSaveHistory. */
...
ZEND_BEGIN_ARG_INFO_EX(gene_db_mysql_quote, 0, 0, 1)
	ZEND_ARG_INFO(0, str)
	ZEND_ARG_INFO(0, paramType)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(gene_db_mysql_transaction, 0, 0, 1)
	ZEND_ARG_CALLABLE_INFO(0, fn, 0)
ZEND_END_ARG_INFO()

void mysql_reset_sql_params(zval *self)
{
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_SQL]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_JOIN]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_WHERE]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_GROUP]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_HAVING]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_UNION]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_ORDER]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_LIMIT]);
	...
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_LOCK]);
    gene_db_prop_set_null(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA]);
}
...
bool mysqlInitPdo (zval * self, zval *config) {
	...
	if (config == NULL) {
		config =  gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_CONFIG]);
	}
    ...
    gene_db_prop_set(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_PDO], &pdo_object);
    zval_ptr_dtor(&pdo_object);
	return 0;
}

bool gene_mysql_pdo_execute (zval *self, zval *statement)
{
	...
	pdo_object = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_PDO]);
	pdo_sql = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_SQL]);
	pdo_join = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_JOIN]);
	pdo_where = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_WHERE]);
	pdo_group = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_GROUP]);
	pdo_having = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_HAVING]);
	pdo_union = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_UNION]);
	pdo_order = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_ORDER]);
	pdo_limit = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_LIMIT]);
	pdo_lock = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_LOCK]);
	...
	if (!GENE_G(run_environment)) {
		markStart(&db_start, &db_mysql_memory_start);
	}

	gene_pdo_prepare(pdo_object, ZSTR_VAL(sql.s), statement);
	if (Z_TYPE_P(statement) == IS_OBJECT) {
		params = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA]);
		//execute
		ZVAL_NULL(&retval);
		...
		if (!GENE_G(run_environment)) {
			markEnd(&db_end, &db_mysql_memory_end);
			mysqlSaveHistory(&sql, params, &db_start, &db_end, &db_mysql_memory_start, &db_mysql_memory_end);
		}
		smart_str_free(&sql);
		{
...
PHP_METHOD(gene_db_mysql, __construct)
{
    ...
    if (config) {
    	gene_db_prop_set(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_CONFIG], config);
...
{
	zval *self = getThis();
	zval *pdo_object = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_PDO]);
 ...
 */
PHP_METHOD(gene_db_mysql, select)
{
    ...
    if (fields) {
    	...
    	case IS_ARRAY:
    		array_to_string(fields, &select, '`', '`');
    		{
    			...
    			GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT %s FROM %s", select, qt);
    			efree(qt);
    		}
            efree(select);
    		break;
    	case IS_STRING:
    		{
    			...
    			GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT %s FROM %s", qf, qt);
    			efree(qf);
    			efree(qt);
    		}
    		break;
    	...
    	char *qt = gene_quote_table(table, '`', '`');
    	GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT * FROM %s", qt);
    	efree(qt);
    }
	RETURN_ZVAL(self, 1, 0);
}
 ...
 */
PHP_METHOD(gene_db_mysql, count)
{
	zval *self = getThis();
	...
	mysql_reset_sql_params(self);
    {
    	...
    	if (fields) {
    		...
    		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT count(%s) AS count FROM %s", qf, qt);
    		efree(qf);
    	} else {
    		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT count(1) AS count FROM %s", qt);
    	}
    	efree(qt);
    }
	RETURN_ZVAL(self, 1, 0);
...
/* Shared INSERT builder. verb selects plain INSERT / INSERT IGNORE. */
static void gene_db_mysql_do_insert(zval *self, char *table, zval *fields, zend_bool ignore)
{
    ...
    if (fields && Z_TYPE_P(fields) == IS_ARRAY) {
    	...
    	gene_db_prop_set(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA], &field_value);
	...
	smart_str_0(&value_str);
    {
    	char *qt = gene_quote_table(table, '`', '`');
    	if (ignore) {
    		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "INSERT IGNORE INTO %s(%s) VALUES(%s)", qt, field_str.s->val, value_str.s->val);
    	} else {
    		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "INSERT INTO %s(%s) VALUES(%s)", qt, field_str.s->val, value_str.s->val);
    	}
    	efree(qt);
    }
 ...
 */
PHP_METHOD(gene_db_mysql, insert)
{
	zval *self = getThis(),*fields = NULL;
	char *table = NULL;
	size_t table_len;// @suppress("Type cannot be resolved")
	...
	gene_db_mysql_do_insert(self, table, fields, 0);
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_mysql::insertIgnore($table, $fields)
 * [GENE_FEATURE:2026-08-18 3.3] Idempotent write (INSERT IGNORE) for
 * task-log / idempotency-key style tables. Lazy like insert(): execute via
 * lastId()/affectedRows(). */
PHP_METHOD(gene_db_mysql, insertIgnore)
{
	zval *self = getThis(),*fields = NULL;
	char *table = NULL;
	size_t table_len;// @suppress("Type cannot be resolved")
	...
	gene_db_mysql_do_insert(self, table, fields, 1);
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_mysql::upsert($table, $fields, array $updateCols)
 * [GENE_FEATURE:2026-08-18 3.3] INSERT ... ON DUPLICATE KEY UPDATE
 * `c`=VALUES(`c`). MySQL-only; other drivers throw. Lazy like insert(). */
PHP_METHOD(gene_db_mysql, upsert)
{
	zval *self = getThis(),*fields = NULL, *update_cols = NULL, *col = NULL;
	char *table = NULL;
	size_t table_len;// @suppress("Type cannot be resolved")
	smart_str field_str = {0} , value_str = {0}, upd_str = {0};
	zval field_value;
	bool pre = 0;
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "s|za", &table, &table_len, &fields, &update_cols) == FAILURE) {
		return;
	}
	if (!update_cols || zend_hash_num_elements(Z_ARRVAL_P(update_cols)) == 0) {
		zend_throw_exception_ex(NULL, 0,
			"Gene\\Db\\Mysql::upsert() requires a non-empty updateCols array");
		return;
	}
	mysql_reset_sql_params(self);
	ZVAL_NULL(&field_value);
    ...
    ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(update_cols), col) {
    	if (Z_TYPE_P(col) != IS_STRING || Z_STRLEN_P(col) == 0) {
    		smart_str_free(&field_str);
    		smart_str_free(&value_str);
    		smart_str_free(&upd_str);
    		zend_throw_exception_ex(NULL, 0,
    			"Gene\\Db\\Mysql::upsert() updateCols entries must be column names");
    		return;
    	}
    	if (pre) {
    		smart_str_appends(&upd_str, ",");
    	} else {
    		pre = 1;
    	}
    	gene_quote_identifier(&upd_str, Z_STRVAL_P(col), Z_STRLEN_P(col), '`', '`');
    	smart_str_appends(&upd_str, "=VALUES(");
    	gene_quote_identifier(&upd_str, Z_STRVAL_P(col), Z_STRLEN_P(col), '`', '`');
    	smart_str_appends(&upd_str, ")");
    } ZEND_HASH_FOREACH_END();
	smart_str_0(&field_str);
    ...
    {
    	char *qt = gene_quote_table(table, '`', '`');
    	GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "INSERT INTO %s(%s) VALUES(%s) ON DUPLICATE KEY UPDATE %s",
    		qt, field_str.s->val, value_str.s->val, upd_str.s->val);
    	efree(qt);
    }
    smart_str_free(&field_str);
    smart_str_free(&value_str);
 ...
 */
PHP_METHOD(gene_db_mysql, batchInsert)
{
 ...
 */
PHP_METHOD(gene_db_mysql, update)
{
	zval *self = getThis(),*fields = NULL;
	char *table = NULL;
	size_t table_len; // @suppress("Type cannot be resolved")
    ...
    {
    	char *qt = gene_quote_table(table, '`', '`');
    	GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "UPDATE %s SET %s", qt, field_str.s->val);
    	efree(qt);
    }
    smart_str_free(&field_str);
	RETURN_ZVAL(self, 1, 0);
 ...
 */
PHP_METHOD(gene_db_mysql, delete)
{
	zval *self = getThis();
	char *table = NULL;
	size_t table_len; // @suppress("Type cannot be resolved")
    ...
    {
    	char *qt = gene_quote_table(table, '`', '`');
    	GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_SQL, "DELETE FROM %s", qt);
    	efree(qt);
    }
	RETURN_ZVAL(self, 1, 0);
}
 ...
 */
PHP_METHOD(gene_db_mysql, where)
{
	...
	mysql_init_where(self, &where_str);

	switch(Z_TYPE_P(where)) {
	case IS_ARRAY:
        data = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA]);
        if (Z_TYPE_P(data) == IS_ARRAY) {
            makeWhere(self, &where_str, where, data);
        } else {
            array_init(&params);
            makeWhere(self, &where_str, where, &params);
            gene_db_prop_set(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA], &params);
    ...
    /* [GENE_PERF:2026-04-26] Pass smart_str's underlying zend_string directly
     * to zend_update_property_str (addref only). Avoids the strdup that
     * zend_update_property_string would do on ZSTR_VAL(where_str.s). */
    gene_db_prop_set_str(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_WHERE], where_str.s);
...
PHP_METHOD(gene_db_mysql, in)
{
	...
	if (in_len) {
		...
		/* [GENE_PERF:2026-04-26] estrndup is a direct memcpy; spprintf("%s",in)
		 * went through smart_string formatting machinery for no benefit. */
    ...
    if (fields) {
    	...
    	case IS_ARRAY:
    		...
    		if (Z_TYPE_P(data) == IS_ARRAY) {
    			...
    			gene_db_prop_set(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA], fields);
    		}
    		smart_str_appends(&value_str, ")");
    		smart_str_0(&value_str);
...
PHP_METHOD(gene_db_mysql, sql)
{
	...
	mysql_reset_sql_params(self);
    if (sql_len) {
        gene_db_prop_set_stringl(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_SQL], sql, sql_len);
    }
    if (fields) {
    	data = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA]);
    	switch(Z_TYPE_P(fields)) {
 ...
 */
PHP_METHOD(gene_db_mysql, execute)
{
	zval *self = getThis();
	zval statement;
	gene_mysql_pdo_execute(self, &statement);
...
static void gene_db_mysql_append_prop(zval *self, const char *prop, size_t prop_len, smart_str *frag) {
	zval *cur = zend_read_property(gene_db_mysql_ce, gene_strip_obj(self), prop, prop_len, 1, NULL);
	smart_str out = {0};
	...
	smart_str_0(&out);
	zend_update_property_str(gene_db_mysql_ce, gene_strip_obj(self), prop, prop_len, out.s);
	zend_string_release(out.s);
}

static void gene_db_mysql_do_join(zval *self, zend_string *table, zval *on, const char *type) {
	smart_str frag = {0}, on_str = {0};
	char tbuf[16];
	const char *ttype = "INNER";
	size_t tlen = 5;
	if (type && type[0]) {
		if (!gene_db_mysql_join_type(type, strlen(type), tbuf, sizeof(tbuf))) {
	...
	if (!gene_db_mysql_build_on(on, &on_str)) {
	...
	}
	{
		...
		smart_str_0(&frag);
		gene_db_mysql_append_prop(self, ZEND_STRL(GENE_DB_MYSQL_JOIN), &frag);
		efree(qt);
	}
	smart_str_free(&on_str);
	smart_str_free(&frag);
...
/*
 * {{{ public gene_db_mysql::reset()
 * [GENE_FEATURE:2026-08-06] Public entry to mysql_reset_sql_params() so one
 * instance can be reused to build multiple statements. */
PHP_METHOD(gene_db_mysql, reset)
{
	zval *self = getThis();
	mysql_reset_sql_params(self);
	RETURN_ZVAL(self, 1, 0);
...
/*
 * {{{ public gene_db_mysql::join(string $table, array $on, string $type = 'INNER')
 */
PHP_METHOD(gene_db_mysql, join)
{
...
/*
 * {{{ public gene_db_mysql::leftJoin(string $table, array $on)
 */
PHP_METHOD(gene_db_mysql, leftJoin)
{
	zval *self = getThis(), *on = NULL;
	...
	gene_db_mysql_do_join(self, table, on, "LEFT");
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_mysql::rightJoin(string $table, array $on)
 */
PHP_METHOD(gene_db_mysql, rightJoin)
{
	zval *self = getThis(), *on = NULL;
	zend_string *table = NULL;
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "Sz", &table, &on) == FAILURE) {
	...
	gene_db_mysql_do_join(self, table, on, "RIGHT");
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_mysql::union(string|object $query, bool $all = false)
 * [GENE_FEATURE:2026-08-06 F0-2] A string is treated as developer-written
 * SQL (same trust level as sql()); a Gene\Db\Mysql object is assembled from
...
PHP_METHOD(gene_db_mysql, union)
{
	...
	case IS_STRING:
		...
		break;
	case IS_OBJECT:
		if (!instanceof_function(Z_OBJCE_P(query), gene_db_mysql_ce)) {
			php_error_docref(NULL, E_WARNING, "union() expects a SQL string or a Gene\\Db\\Mysql builder");
			smart_str_free(&frag);
			return;
		}
		{
			zval *sub_sql = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_SQL]);
			zval *sub_join = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_JOIN]);
			zval *sub_where = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_WHERE]);
			zval *sub_group = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_GROUP]);
			zval *sub_having = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_HAVING]);
			zval *sub_order = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_ORDER]);
			zval *sub_limit = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_LIMIT]);
			zval *sub_data = gene_db_prop_get(gene_strip_obj(query), gene_db_mysql_prop[GENE_DB_PROP_DATA]);
			smart_str_appendc(&frag, '(');
			...
			if (sub_data && Z_TYPE_P(sub_data) == IS_ARRAY && zend_hash_num_elements(Z_ARRVAL_P(sub_data)) > 0) {
				zval *data = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_DATA]);
				zval *value;
				if (Z_TYPE_P(data) == IS_ARRAY) {
	...
	smart_str_0(&frag);
	gene_db_mysql_append_prop(self, ZEND_STRL(GENE_DB_MYSQL_UNION), &frag);
	smart_str_free(&frag);
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
 ...
 */
PHP_METHOD(gene_db_mysql, group)
{
	zval *self = getThis();
	char *group = NULL;
	...
	if (group_len) {
		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_GROUP, " GROUP BY %s", group);
	}
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
 ...
 */
PHP_METHOD(gene_db_mysql, having)
{
	zval *self = getThis();
	char *having = NULL;
	...
	if (having_len) {
		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_HAVING, " HAVING %s", having);
	}
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
 ...
 */
PHP_METHOD(gene_db_mysql, order)
{
	zval *self = getThis();
	char *order = NULL;
	...
	if (order_len) {
		...
		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_ORDER, " ORDER BY %s", qo);
		efree(qo);
	}
	RETURN_ZVAL(self, 1, 0);
}
 ...
 */
PHP_METHOD(gene_db_mysql, limit)
{
	zval *self = getThis();
	...
	if (ZEND_NUM_ARGS() > 1) {
		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_LIMIT, " LIMIT " ZEND_LONG_FMT "," ZEND_LONG_FMT, num, offset);
	} else {
		GENE_DB_MYSQL_SET_PROP(GENE_DB_PROP_LIMIT, " LIMIT " ZEND_LONG_FMT, num);
	}
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
...
static void gene_db_mysql_set_lock(zval *self, const char *frag)
{
	...
	gene_db_prop_set_string(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_LOCK], frag);
}

/*
 * {{{ public gene_db_mysql::lockForUpdate()
 */
PHP_METHOD(gene_db_mysql, lockForUpdate)
{
	zval *self = getThis();
	gene_db_mysql_set_lock(self, " FOR UPDATE");
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_mysql::sharedLock()
 */
PHP_METHOD(gene_db_mysql, sharedLock)
{
	zval *self = getThis();
	gene_db_mysql_set_lock(self, " LOCK IN SHARE MODE");
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
 ...
 */
PHP_METHOD(gene_db_mysql, lastId)
{
 ...
 */
PHP_METHOD(gene_db_mysql, affectedRows)
{
	zval *self = getThis();
	zval statement;
	if (gene_mysql_pdo_execute(self, &statement)) {
 ...
 */
PHP_METHOD(gene_db_mysql, quote)
{
	zval *self = getThis(), *pdo_object = NULL;
 ...
 */
PHP_METHOD(gene_db_mysql, print)
{
 ...
 */
PHP_METHOD(gene_db_mysql, transaction)
{
	zend_fcall_info fci;
...
{
	zval *self = getThis();
	zval *pool = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_POOL]);
 ...
 */
PHP_METHOD(gene_db_mysql, __destruct)
{
	zval *self = getThis();
	zval *pool = gene_db_prop_get(gene_strip_obj(self), gene_db_mysql_prop[GENE_DB_PROP_POOL]);
	if (pool && Z_TYPE_P(pool) == IS_OBJECT) {
 ...
 */
const zend_function_entry gene_db_mysql_methods[] = {
		PHP_ME(gene_db_mysql, __construct, gene_db_mysql_construct, ZEND_ACC_PUBLIC)
		...
		PHP_ME(gene_db_mysql, insertIgnore, gene_db_mysql_insert, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, upsert, gene_db_mysql_upsert, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, batchInsert, gene_db_mysql_batch_insert, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, update, gene_db_mysql_update, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, delete, gene_db_mysql_delete, ZEND_ACC_PUBLIC)
		...
		PHP_ME(gene_db_mysql, limit, gene_db_mysql_limit, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, lockForUpdate, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, sharedLock, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		...
		PHP_MALIAS(gene_db_mysql, lastInsertId, lastId, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, affectedRows, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_MALIAS(gene_db_mysql, rowCount, affectedRows, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, quote, gene_db_mysql_quote, ZEND_ACC_PUBLIC)
		...
		PHP_ME(gene_db_mysql, transaction, gene_db_mysql_transaction, ZEND_ACC_PUBLIC)
		PHP_MALIAS(gene_db_mysql, transact, transaction, gene_db_mysql_transaction, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, release, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, free, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_mysql, __destruct, gene_db_mysql_void_arginfo, ZEND_ACC_PUBLIC)
...
GENE_MINIT_FUNCTION(db_mysql)
{
	...
	gene_db_mysql_ce->ce_flags |= ZEND_ACC_ALLOW_DYNAMIC_PROPERTIES;
#endif
    ...
    zend_declare_property_null(gene_db_mysql_ce, ZEND_STRL(GENE_DB_MYSQL_JOIN), ZEND_ACC_PUBLIC);
    ...
    zend_declare_property_null(gene_db_mysql_ce, ZEND_STRL(GENE_DB_MYSQL_UNION), ZEND_ACC_PUBLIC);
    ...
    zend_declare_property_null(gene_db_mysql_ce, ZEND_STRL(GENE_DB_MYSQL_LOCK), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_mysql_ce, ZEND_STRL(GENE_DB_MYSQL_DATA), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_mysql_ce, ZEND_STRL(GENE_DB_MYSQL_POOL), ZEND_ACC_PROTECTED);
    zend_declare_property_null(gene_db_mysql_ce, ZEND_STRL(GENE_DB_MYSQL_HISTORY), ZEND_ACC_PROTECTED | ZEND_ACC_STATIC);

	gene_db_prop_offsets_init(gene_db_mysql_ce, gene_db_mysql_prop);
