...
static uint32_t gene_db_pgsql_prop[GENE_DB_PROP_N];
...
#define GENE_DB_PGSQL_SET_PROP(KEY, ...) do { \
    zend_string *_gene_db_s_ = strpprintf(0, __VA_ARGS__); \
    gene_db_prop_set_str(gene_strip_obj(self), gene_db_pgsql_prop[KEY], _gene_db_s_); \
    zend_string_release(_gene_db_s_); \
} while (0)

/* Benchmark variables removed from file scope to avoid coroutine data races.
 * They are now local to gene_pgsql_pdo_execute and passed to pgsqlSaveHistory. */
...
ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_insert, 0, 0, 1)
	ZEND_ARG_INFO(0, table)
    ZEND_ARG_INFO(0, fields)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_batch_insert, 0, 0, 1)
	ZEND_ARG_INFO(0, table)
    ZEND_ARG_INFO(0, fields)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_update, 0, 0, 1)
	ZEND_ARG_INFO(0, table)
    ZEND_ARG_INFO(0, fields)
ZEND_END_ARG_INFO()
...
ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_join, 0, 0, 2)
	ZEND_ARG_INFO(0, table)
	ZEND_ARG_INFO(0, on)
	ZEND_ARG_INFO(0, type)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_arithmetic_arginfo, 0, 0, 2)
	ZEND_ARG_INFO(0, table)
	ZEND_ARG_INFO(0, column)
	ZEND_ARG_INFO(0, amount)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_side_join, 0, 0, 2)
	ZEND_ARG_INFO(0, table)
	ZEND_ARG_INFO(0, on)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_union, 0, 0, 1)
	ZEND_ARG_INFO(0, query)
	ZEND_ARG_INFO(0, all)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(gene_db_pgsql_quote, 0, 0, 1)
	ZEND_ARG_INFO(0, str)
...
{
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_SQL]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_JOIN]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_WHERE]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_GROUP]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_HAVING]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_UNION]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_ORDER]);
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_LIMIT]);
	...
	gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_LOCK]);
    gene_db_prop_set_null(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA]);
}
...
void pgsql_init_where(zval *self, smart_str *where_str) {
	zval *pdo_where = NULL;
	pdo_where = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_WHERE]);
	if (pdo_where) {
...
bool pgsqlInitPdo (zval * self, zval *config) {
	zval  *dsn = NULL, *user = NULL, *pass = NULL, *options = NULL;
	zval pdo_object, option;

	if (config == NULL) {
		config =  gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_CONFIG]);
	}
	...
	if (gene_pool_get_pdo(gene_db_pgsql_ce, self, config, ZEND_STRL(GENE_DB_PGSQL_POOL), ZEND_STRL(GENE_DB_PGSQL_PDO))) {
		return 0;
	}
    ...
    gene_db_prop_set(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_PDO], &pdo_object);
    zval_ptr_dtor(&pdo_object);
	return 0;
}
...
bool gene_pgsql_pdo_execute (zval *self, zval *statement)
{
	...
	pdo_object = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_PDO]);
	pdo_sql = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_SQL]);
	pdo_join = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_JOIN]);
	pdo_where = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_WHERE]);
	pdo_group = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_GROUP]);
	pdo_having = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_HAVING]);
	pdo_union = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_UNION]);
	pdo_order = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_ORDER]);
	pdo_limit = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_LIMIT]);
	pdo_lock = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_LOCK]);
	...
	if (Z_TYPE_P(statement) == IS_OBJECT) {
		params = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA]);
		//execute
		ZVAL_NULL(&retval);
    	...
    	if (EG(exception)) {
    		if (checkPdoError(EG(exception))) {
    			...
    			gene_pool_notify_remove(gene_db_pgsql_ce, self, ZEND_STRL(GENE_DB_PGSQL_POOL));
		...
		if (!GENE_G(run_environment)) {
			...
			pgsqlSaveHistory(&sql, params, &db_start, &db_end, &db_pgsql_memory_start, &db_pgsql_memory_end);
		}
		smart_str_free(&sql);
		{
...
PHP_METHOD(gene_db_pgsql, __construct)
{
    ...
    if (config) {
    	gene_db_prop_set(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_CONFIG], config);
...
{
	zval *self = getThis();
	zval *pdo_object = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_PDO]);
    ...
    if (fields) {
    	...
    	case IS_ARRAY:
    		mssql_array_to_string(fields, &select, '"', '"');
    		{
    			...
    			GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT %s FROM %s", select, qt);
    			efree(qt);
    		}
            efree(select);
    		break;
    	case IS_STRING:
    		{
    			...
    			GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT %s FROM %s", qf, qt);
    			efree(qf);
    			efree(qt);
    		}
    		break;
    	...
    	char *qt = gene_quote_table(table, '"', '"');
    	GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT * FROM %s", qt);
    	efree(qt);
    }
	RETURN_ZVAL(self, 1, 0);
}
...
PHP_METHOD(gene_db_pgsql, count)
{
	...
	pgsql_reset_sql_params(self);
    {
    	...
    	if (fields) {
    		...
    		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT count(%s) AS count FROM %s", qf, qt);
    		efree(qf);
    	} else {
    		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "SELECT count(1) AS count FROM %s", qt);
    	}
    	efree(qt);
    }
	RETURN_ZVAL(self, 1, 0);
...
PHP_METHOD(gene_db_pgsql, insert)
{
    ...
    if (fields && Z_TYPE_P(fields) == IS_ARRAY) {
    	...
    	gene_db_prop_set(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA], &field_value);
    ...
    {
    	char *qt = gene_quote_table(table, '"', '"');
    	GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "INSERT INTO %s(%s) VALUES(%s)", qt, field_str.s->val, value_str.s->val);
    	efree(qt);
    }
...
PHP_METHOD(gene_db_pgsql, update)
{
    ...
    {
    	char *qt = gene_quote_table(table, '"', '"');
    	GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "UPDATE %s SET %s", qt, field_str.s->val);
    	efree(qt);
    }
    smart_str_free(&field_str);
	RETURN_ZVAL(self, 1, 0);
...
PHP_METHOD(gene_db_pgsql, delete)
{
    ...
    {
    	char *qt = gene_quote_table(table, '"', '"');
    	GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_SQL, "DELETE FROM %s", qt);
    	efree(qt);
    }
	RETURN_ZVAL(self, 1, 0);
}
...
PHP_METHOD(gene_db_pgsql, where)
{
	...
	case IS_ARRAY:
        data = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA]);
        if (Z_TYPE_P(data) == IS_ARRAY) {
        	makeWhere(self, &where_str, where, data);
        } else {
            array_init(&params);
            makeWhere(self, &where_str, where, &params);
            gene_db_prop_set(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA], &params);
    ...
    gene_db_prop_set_str(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_WHERE], where_str.s);
    ...
    if (fields) {
    	...
    	case IS_ARRAY:
    		...
    		if (Z_TYPE_P(data) == IS_ARRAY) {
    			...
    			gene_db_prop_set(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA], fields);
    		}
    		smart_str_appends(&value_str, ")");
    		smart_str_0(&value_str);
...
PHP_METHOD(gene_db_pgsql, sql)
{
	...
	pgsql_reset_sql_params(self);
    if (sql_len) {
        gene_db_prop_set_stringl(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_SQL], sql, sql_len);
    }
    if (fields) {
    	data = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA]);
    	switch(Z_TYPE_P(fields)) {
...
static void gene_db_pgsql_append_prop(zval *self, const char *prop, size_t prop_len, smart_str *frag) {
	zval *cur = zend_read_property(gene_db_pgsql_ce, gene_strip_obj(self), prop, prop_len, 1, NULL);
	smart_str out = {0};
	...
	smart_str_0(&out);
	zend_update_property_str(gene_db_pgsql_ce, gene_strip_obj(self), prop, prop_len, out.s);
	zend_string_release(out.s);
}

static void gene_db_pgsql_do_join(zval *self, zend_string *table, zval *on, const char *type) {
	smart_str frag = {0}, on_str = {0};
	char tbuf[16];
	const char *ttype = "INNER";
	size_t tlen = 5;
	if (type && type[0]) {
		if (!gene_db_pgsql_join_type(type, strlen(type), tbuf, sizeof(tbuf))) {
	...
	if (!gene_db_pgsql_build_on(on, &on_str)) {
	...
	}
	{
		...
		smart_str_0(&frag);
		gene_db_pgsql_append_prop(self, ZEND_STRL(GENE_DB_PGSQL_JOIN), &frag);
		efree(qt);
	}
	smart_str_free(&on_str);
	smart_str_free(&frag);
...
/*
 * {{{ public gene_db_pgsql::reset()
 * [GENE_FEATURE:2026-08-06] Public entry to pgsql_reset_sql_params() so one
 * instance can be reused to build multiple statements. */
PHP_METHOD(gene_db_pgsql, reset)
{
	zval *self = getThis();
	pgsql_reset_sql_params(self);
	RETURN_ZVAL(self, 1, 0);
...
/*
 * {{{ public gene_db_pgsql::join(string $table, array $on, string $type = 'INNER')
 */
PHP_METHOD(gene_db_pgsql, join)
{
	...
	gene_db_pgsql_do_join(self, table, on, (type && ZSTR_LEN(type)) ? ZSTR_VAL(type) : NULL);
	RETURN_ZVAL(self, 1, 0);
}

PHP_METHOD(gene_db_pgsql, joinOn)
{
	zval *self = getThis(), *predicates = NULL;
	zend_string *table = NULL, *type = NULL;
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "Sa|S", &table, &predicates, &type) == FAILURE) return;
	if (gene_db_build_join_on(self, gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_JOIN), ZEND_STRL(GENE_DB_PGSQL_DATA), table, predicates, type, '"', '"') != SUCCESS) RETURN_NULL();
	RETURN_ZVAL(self, 1, 0);
}

static void gene_db_pgsql_arithmetic(INTERNAL_FUNCTION_PARAMETERS, zend_bool increment)
{
	...
	pgsql_reset_sql_params(self);
	if (gene_db_build_arithmetic(self, gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_SQL), ZEND_STRL(GENE_DB_PGSQL_DATA), table, column, value, is_long, long_value, increment, '"', '"') != SUCCESS) RETURN_NULL();
	RETURN_ZVAL(self, 1, 0);
}

PHP_METHOD(gene_db_pgsql, increment) { gene_db_pgsql_arithmetic(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1); }
PHP_METHOD(gene_db_pgsql, decrement) { gene_db_pgsql_arithmetic(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0); }
/* }}} */

/*
 * {{{ public gene_db_pgsql::leftJoin(string $table, array $on)
 */
PHP_METHOD(gene_db_pgsql, leftJoin)
{
	zval *self = getThis(), *on = NULL;
	...
	gene_db_pgsql_do_join(self, table, on, "LEFT");
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_pgsql::rightJoin(string $table, array $on)
 */
PHP_METHOD(gene_db_pgsql, rightJoin)
{
	zval *self = getThis(), *on = NULL;
	zend_string *table = NULL;
	if (zend_parse_parameters(ZEND_NUM_ARGS(), "Sz", &table, &on) == FAILURE) {
	...
	gene_db_pgsql_do_join(self, table, on, "RIGHT");
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */

/*
 * {{{ public gene_db_pgsql::union(string|object $query, bool $all = false)
 * [GENE_FEATURE:2026-08-06 F0-2] A string is treated as developer-written
 * SQL (same trust level as sql()); a Gene\Db\Pgsql object is assembled from
...
PHP_METHOD(gene_db_pgsql, union)
{
	...
	case IS_STRING:
		...
		break;
	case IS_OBJECT:
		if (!instanceof_function(Z_OBJCE_P(query), gene_db_pgsql_ce)) {
			php_error_docref(NULL, E_WARNING, "union() expects a SQL string or a Gene\\Db\\Pgsql builder");
			smart_str_free(&frag);
			return;
		}
		{
			zval *sub_sql = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_SQL]);
			zval *sub_join = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_JOIN]);
			zval *sub_where = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_WHERE]);
			zval *sub_group = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_GROUP]);
			zval *sub_having = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_HAVING]);
			zval *sub_order = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_ORDER]);
			zval *sub_limit = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_LIMIT]);
			zval *sub_data = gene_db_prop_get(gene_strip_obj(query), gene_db_pgsql_prop[GENE_DB_PROP_DATA]);
			smart_str_appendc(&frag, '(');
			...
			if (sub_data && Z_TYPE_P(sub_data) == IS_ARRAY && zend_hash_num_elements(Z_ARRVAL_P(sub_data)) > 0) {
				zval *data = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_DATA]);
				zval *value;
				if (Z_TYPE_P(data) == IS_ARRAY) {
					SEPARATE_ARRAY(data);
	...
	smart_str_0(&frag);
	gene_db_pgsql_append_prop(self, ZEND_STRL(GENE_DB_PGSQL_UNION), &frag);
	smart_str_free(&frag);
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
...
PHP_METHOD(gene_db_pgsql, group)
{
	...
	if (group_len) {
		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_GROUP, " GROUP BY %s", group);
	}
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
...
PHP_METHOD(gene_db_pgsql, having)
{
	...
	if (having_len) {
		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_HAVING, " HAVING %s", having);
	}
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
...
PHP_METHOD(gene_db_pgsql, order)
{
	...
	if (order_len) {
		...
		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_ORDER, " ORDER BY %s", qo);
		efree(qo);
	}
	RETURN_ZVAL(self, 1, 0);
}
...
PHP_METHOD(gene_db_pgsql, limit)
{
	...
	if (ZEND_NUM_ARGS() > 1) {
		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_LIMIT, " limit " ZEND_LONG_FMT " offset " ZEND_LONG_FMT, num, offset);
	} else {
		GENE_DB_PGSQL_SET_PROP(GENE_DB_PROP_LIMIT, " limit " ZEND_LONG_FMT, num);
	}
	RETURN_ZVAL(self, 1, 0);
}
/* }}} */
...
static void gene_db_pgsql_set_lock(zval *self, const char *frag)
{
	...
	gene_db_prop_set_string(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_LOCK], frag);
}
 ...
 */
PHP_METHOD(gene_db_pgsql, quote)
{
	zval *self = getThis(), *pdo_object = NULL;
...
{
	zval *self = getThis();
	zval *pool = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_POOL]);
 ...
 */
PHP_METHOD(gene_db_pgsql, __destruct)
{
	zval *self = getThis();
	zval *pool = gene_db_prop_get(gene_strip_obj(self), gene_db_pgsql_prop[GENE_DB_PROP_POOL]);
	if (pool && Z_TYPE_P(pool) == IS_OBJECT) {
 ...
 */
const zend_function_entry gene_db_pgsql_methods[] = {
		PHP_ME(gene_db_pgsql, __construct, gene_db_pgsql_construct, ZEND_ACC_PUBLIC)
		...
		PHP_ME(gene_db_pgsql, join, gene_db_pgsql_join, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, joinOn, gene_db_pgsql_join, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, increment, gene_db_pgsql_arithmetic_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, decrement, gene_db_pgsql_arithmetic_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, leftJoin, gene_db_pgsql_side_join, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, rightJoin, gene_db_pgsql_side_join, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, union, gene_db_pgsql_union, ZEND_ACC_PUBLIC)
		...
		PHP_MALIAS(gene_db_pgsql, lastInsertId, lastId, gene_db_pgsql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, affectedRows, gene_db_pgsql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_MALIAS(gene_db_pgsql, rowCount, affectedRows, gene_db_pgsql_void_arginfo, ZEND_ACC_PUBLIC)
		PHP_ME(gene_db_pgsql, quote, gene_db_pgsql_quote, ZEND_ACC_PUBLIC)
...
GENE_MINIT_FUNCTION(db_pgsql)
{
	...
	gene_db_pgsql_ce->ce_flags |= ZEND_ACC_ALLOW_DYNAMIC_PROPERTIES;
#endif

	//pdo
    zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_CONFIG), ZEND_ACC_PUBLIC);
	zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_PDO), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_SQL), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_JOIN), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_WHERE), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_GROUP), ZEND_ACC_PUBLIC);
    zend_declare_property_null(gene_db_pgsql_ce, ZEND_STRL(GENE_DB_PGSQL_HAVING), ZEND_ACC_PUBLIC);
	...
	gene_db_prop_offsets_init(gene_db_pgsql_ce, gene_db_pgsql_prop);
