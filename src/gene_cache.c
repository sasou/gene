...
static void gene_hash_copy(HashTable *dst, HashTable *source) /* {{{ */ {
	...
	ZEND_HASH_FOREACH_KEY_VAL(source, idx, key, element) {
		...
		if (key) {
			gene_symtable_update(dst, gene_str_persistent(ZSTR_VAL(key), ZSTR_LEN(key)), &rv);
		} else {
...
static void gene_cache_hash_copy(HashTable *target, HashTable *source) /* {{{ */ {
	...
	ZEND_HASH_FOREACH_KEY_VAL(source, idx, key, element) {
		gene_cache_zval_persistent(&rv, element);
		if (key) {
			gene_symtable_update(target, gene_str_persistent(ZSTR_VAL(key), ZSTR_LEN(key)), &rv);
		} else {
...
void gene_cache_hash_copy_local(HashTable *target, HashTable *source) /* {{{ */ {
	...
	ZEND_HASH_FOREACH_KEY_VAL(source, idx, key, element) {
		...
		if (key) {
			gene_symtable_update(target, zend_string_init(ZSTR_VAL(key), ZSTR_LEN(key), 0), &rv);
		} else {
			zend_hash_index_update(target, idx, &rv);
		}
	} ZEND_HASH_FOREACH_END();
...
PHP_METHOD(gene_cache, __construct)
{
    ...
    if(safe) {
    	zend_update_property_string(gene_cache_ce, getThis(), GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), Z_STRVAL_P(safe));
    } else {
    	if (GENE_G(app_key)) {
    		zend_update_property_string(gene_cache_ce, getThis(), GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), GENE_G(app_key));
    	} else {
    		gene_ini_router();
    		zend_update_property_string(gene_cache_ce, getThis(), GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), GENE_G(directory));
    	}
    }
}
/* }}} */
...
GENE_MINIT_FUNCTION(cache)
{
	...
	//debug
    zend_declare_property_string(gene_cache_ce, GENE_CACHE_SAFE, strlen(GENE_CACHE_SAFE), "", ZEND_ACC_PUBLIC);
    //
	return SUCCESS;
}
/* }}} */
