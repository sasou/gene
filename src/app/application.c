...
/*
 * {{{ void load_file(char *key, size_t key_len,char *php_script, zend_long validity)
 */
void load_file(char *key, size_t key_len, char *php_script, zend_long validity) {
 ...
 *          opts in): the collector walks all live zvals and costs ��s-ms
...
PHP_METHOD(gene_application, workerReady) {
	 ...
	 * is ever reached, so eviction never triggers and new business keys land
	 * in an unbounded table — the Gene\Cache layer degrades to permanent
	 * growth. (The old cache_insert_refused guard was never real; R11.)
 ...
 *
 *   waitWorkerReady �� Request::initSwoole �� setResponse �� ���� ob buffer
 *   �� run() �� Throwable �߽� �� ����������� �� δ������ response->end(output)
 *   �� cleanup(cleanup_gc)
 *
 * Options: ['cleanup_gc' => false, 'catch' => null]. catch Ϊ
 * function (\Throwable $e, object $request, object $response): void��
 * δ����ʱ��ܼ� Gene\Log::exception() ������Ӧ�Կ�д��δ����ʱ��
 * status(500) + end(buffered output)��catch �����쳣��buffer �ָ���
 * cleanup �ճ�ִ�У��쳣���⴫����������Ĭ�� Content-Type��������ҵ��
 * status/header��dispatch ���� end/json/sendFile/redirect �������������
 * end()��run() �Ľ��� auto-cleanup �ڴ˱� swoole_handle_depth ���ƣ�
 * �ɱ�������ռ cleanup ʱ���� */
PHP_METHOD(gene_application, handleSwoole) {
	...
	if (UNEXPECTED(EG(exception))) {
		...
		if (catch_cb) {
			...
			zval_ptr_dtor(&cret);
			/* catch ���ף�EG(exception) ���¹��� �� ���� end���ճ��ָ�+cleanup�� */
		} else {
	...
	zend_bool sent = 0;
	if (!EG(exception)) {
		/* �տ��ж�ֻ�� Swoole ���д�ԣ����� ctx->response_ended��
		 * Response::end/redirect/sendFile ֱ�� Swoole �� isWritable()=false
		 * ����Ȼ������ end������ Response::json �� php_write �� buffer ����
		 * ended ��ǣ�response �����Կ�д��buffered body ���������﷢��ȥ�� */
		zval *swoole_resp = gene_response_context_obj();
		if (swoole_resp) {
			/* isWritable() ���ݲ����� Response::isSent() һ�£�
			 * �������ɽ���ʱ��"δ����"���������� end() ���ס� */
	...
	/* ���ռ����������dispatch �ڼ�ҵ��й©��Ƕ�� buffer �� end ������
	 * ���㣨ob_end_flush ���壩���������ǰ���е���� buffer ��������
	 * ����ȡ�����ݺ� discard����ת������㡣 */
 ...
 * [GENE_FEATURE:2026-09-12] FPM/Swoole ������Ӧ��װ���տڣ��ȼ��ڣ�
 *   autoload($appRoot)
 *   ->load($router, $confDir)            // options['router']���ɺ� {env}
 *   ->load($config, $confDir)            // options['config']��{env} չ��Ϊ
 *                                      //   getEnvironmentName()
 *   ->setMode($mode ?? 1, $debug, $ex_callback, $error_callback)
 *                                      // $debug = options['debug'] ?? (env �� options['debug_envs'])
 *
 * ������ request-id/webscan/ʱ��/����/ҵ���쳣�ŷ� ���� ��Щ��Ӧ����ʽ���á�
 */
PHP_METHOD(gene_application, bootstrap) {
	...
	if (debug_v && Z_TYPE_P(debug_v) != IS_NULL) {
		/* ��ʽ debug ������ debug_envs ����ƥ�䣨�ȼ۾�ʽ setMode(1,1) �㿪�� */
	...
	if (err_cb && Z_TYPE_P(err_cb) != IS_NULL) {
		if (mode_argc == 2) {
			/* ex_callback ȱʡռλ����ʽ�� Gene �����쳣����������
			 * �ȼ� setMode ������ȱʡʱ gene_exception_register(NULL) �Ļ��ˣ�
			 * ���ܴ� NULL zval��set_exception_handler(NULL) ��ж������������ */
...
/* [GENE_FEATURE:2026-09-12] Pool �����ǼǱ���worker ��������ȫ�ֱ�����
 * ��ʽ���������� config component������ɨ�����ò²����͡� */
 ...
 * [GENE_FEATURE:2026-09-12] �Ǽ����ӳ�������
 *   ['dbPool' => ['driver' => 'db',    'component' => 'db',    'params' => [...]],
 *    'redisPool' => ['driver' => 'redis', 'component' => 'redis']]
 * driver ������ 'db'��Gene\Pool���� 'redis'��Gene\Cache\RedisPool����
 * component Ϊ config ������params Ϊ��ѡ�ز�����min/max/idleTimeout/
 * waitTimeout�����������ĳز�����������ValueError����
 */
...
/* [GENE_FEATURE:2026-09-12] Pool ����ǰ������Ԥ�� ���� �� Pool::create /
 * RedisPool::create ��ͬ�� "<app_key|app_root>:config" ���ҡ�����ȱʧʱ
 * ��ʽ���쳣�����û��ش��ſ� params ע�ᣨTimer Ҳ������������ */
 ...
 * [GENE_FEATURE:2026-09-12] ����������ȫ��δ�����ĳأ�workerStart �е��ã���
 * FPM��runtime_type < 2������ȷ�ܾ����� false���ظ������ݵȣ�ĳ���ش���
 * ʧ��ʱ�쳣���⴫���Ҹ���������δ�����������ԣ��������°��ʼ��ע���
 */
 ...
 * [GENE_FEATURE:2026-09-12] workerExit �ã���Ϊ"��ͨ�� startPools ����"��
 * ��������� stopTimers()����Ӱ��δ����/�ֶ������ĳء��ݵȡ�
 */
 ...
 * [GENE_FEATURE:2026-09-12] workerStop �ã�Ϊ�����������������
 * closeAll()�����λ started ���ʹ���̿ɰ�ȫ���루�ݵȣ���
 */
...
const zend_function_entry gene_application_methods[] = {
	...
	/* [GENE_FEATURE:2026-09-12] Swoole onRequest ���������տ� ���� see impl. */
	PHP_ME(gene_application, handleSwoole, gene_application_handle_swoole, ZEND_ACC_PUBLIC)
	/* [GENE_FEATURE:2026-09-12] FPM/Swoole ����װ���տ� + ��ʽ Pool ���š� */
