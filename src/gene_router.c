...
int setMca(char key, char *val TSRMLS_DC)
{
	...
	switch(key) {
	    case 'm':
	    	GENE_G(module) = estrndup(val, strlen(val));
	    	ZVAL_STRING(&sval, val);
	    	zend_hash_str_update(&EG(symbol_table), "m", 1, &sval);
	    	break;
	    case 'c':
	    	GENE_G(controller) = estrndup(val, strlen(val));
	    	ZVAL_STRING(&sval, val);
	    	zend_hash_str_update(&EG(symbol_table), "c", 1, &sval);
		    break;
	    case 'a':
	    	GENE_G(action) = estrndup(val, strlen(val));
	    	ZVAL_STRING(&sval, val);
...
char * get_function_content(zval *content TSRMLS_DC)
{
	...
	zend_call_method_with_1_params(&objEx, NULL, NULL, "__construct", NULL, content);
	zend_call_method_with_0_params(&objEx, NULL, NULL, "getFileName", &fileName);
	...
	zend_call_method_with_1_params(&objEx, NULL, NULL, "__construct", NULL, &fileName);
	...
	zend_call_method_with_2_params(NULL, NULL, NULL, "strpos", &ret, &arg1, &fileName);
	...
	zend_call_method_with_2_params(NULL, NULL, NULL, "strrpos", &ret, &arg1, &fileName);
	endline = Z_LVAL(ret);
	zval_ptr_dtor(&ret);
	zval_ptr_dtor(&fileName);
	zval_ptr_dtor(&arg);
...
PHP_METHOD(gene_router, run)
{
	...
	safe = zend_read_property(gene_router_ce, self, GENE_ROUTER_SAFE, strlen(GENE_ROUTER_SAFE), 1, NULL);
...
PHP_METHOD(gene_router, __call)
{
	...
	long methodlen;
	int i,router_e_len;
	char *result=NULL,*tmp = NULL,*router_e,*key = NULL,*method,*path = NULL;
		    ...
		    if(pathVal) {
			    ...
			    for(i=0;i<9;i++) {
			    	if (strcmp(methods[i], method)==0) {
			    		prefix = zend_read_property(gene_router_ce, self, GENE_ROUTER_PREFIX, strlen(GENE_ROUTER_PREFIX), 1, NULL);
		    ...
		    for(i=0;i<9;i++) {
		    	if (strcmp(methods[i], method)==0) {
		    		...
		    		if (strlen(path) == 0) {
			    		...
			    		gene_cache_set_by_router(router_e, router_e_len, key, &pathvals, 0 TSRMLS_CC);
			    		efree(key);
...
PHP_METHOD(gene_router, match)
{
	...
	zval result_match,ret_long,*self = getThis();
	zval *data;
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC,"S", &fileName) == FAILURE)
    {
        RETURN_NULL();
    ...
    rec = readfilecontent(ZSTR_VAL(fileName));
    if (rec != NULL) {
    	zend_string *reg = zend_string_init(GENE_ROUTER_CONTENT_REG, strlen(GENE_ROUTER_CONTENT_REG), 0);
    	pce = pcre_get_compiled_regex_cache(reg);
    	if(pce != NULL ){
    		array_init(&result_match);
    		php_pcre_match_impl(pce, rec, strlen(rec), &ret_long, &result_match, 1, 0, 0, 0 TSRMLS_CC);
			efree(rec);
			zval_ptr_dtor(&ret_long);
			data = zend_hash_index_find((&result_match)->value.arr, 0);
			if(data) {
				zval_ptr_dtor(&result_match);
				ZVAL_COPY_VALUE(return_value, data);
				return;
			}
    	}
    	efree(rec);
