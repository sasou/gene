...
ZEND_BEGIN_ARG_INFO_EX(gene_controller_get_path, 0, 0, 0)
...
/** {{{ public gene_controller::getPath([bool $withoutLang = false])
 *  返回当前请求路径。$withoutLang=true 时去除语言前缀。
 */
PHP_METHOD(gene_controller, getPath) {
...
/** {{{ public gene_controller::getRouterUri()
 *  返回当前路由 URI（:m/:c/:a 替换后，小写）。
 */
PHP_METHOD(gene_controller, getRouterUri) {
	gene_get_router_uri(return_value);
}
/* }}} */

/** {{{ public gene_controller::url(string $path [, string $lang])
...
const zend_function_entry gene_controller_methods[] = {
	...
	PHP_ME(gene_controller, input, gene_controller_param_get, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	...
	PHP_ME(gene_controller, getPath, gene_controller_get_path, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
	PHP_ME(gene_controller, getRouterUri, gene_controller_void_arginfo, ZEND_ACC_PUBLIC|ZEND_ACC_STATIC)
