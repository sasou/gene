# Gene 扩展极致并发优化 —— V2：后续优化与增强验证

> 版本：v6-close（2026-09-08）。本文件由原 `PERFORMANCE_OPTIMIZATION.md`（v6，2026-09-06）
> 按执行状态拆分：
> - **V1（`PERFORMANCE_OPTIMIZATION_V1.md`）**：第一批实现已于 2026-09-08 关闭，支持矩阵为 PHP 8.1 NTS。
> - **V2（本文件）**：尚未开始的优化、专项设计，以及 V1 移交的非阻塞增强验证和平台扩展。
>
...
这些项目不影响 V1 已有 NTS 功能关闭结论，也不得在没有数据时反向补写性能收益；若发现正确性
缺陷则新建立项修复：

1. **平台扩展**：决定是否正式支持 ZTS；若支持，治理文件作用域 `zend_function*`、
   `zend_class_entry*`、`HashTable*` 的共享与初始化，并建立 ZTS 构建/多线程 SAPI 矩阵。
2. **跨请求兼容**：opcache 关闭和 `opcache.file_cache_only=1` 下，同一 FPM worker 连续请求回归。
3. **生命周期矩阵**：route_pc clear/dispatch 协程交错、业务缓存 rehash/churn、FPM 同 worker、
   `pcntl_fork`、Swoole 多 worker/workerStop 与 Pool close 交错。
4. **规模与收益量化**：1/32/128/512/1024 并发梯度；池低负载/突发扩容/满池排队；本地
   SQLite/Redis ns/op 与 p50/p95/p99；高 churn 和 worker RSS 趋势。
5. **补充诊断**：Linux ASAN 建议执行，但不是里程碑关闭门槛。ASAN 结果只覆盖实际执行路径，
   不能替代所有权审查、功能回归或并发矩阵。
6. **观测补齐**：框架/业务缓存锁路径、池等待时长、route_pc hit/miss；具体指标见 §5.2。

---

## 2. 轨道 A：D1/D2 —— 每协程成本与不可让出区间（未开始部分）

> §2.1–§2.3 已实施并随 V1 关闭；收益量化与增强验证由本文件 §1 承接。
...
> §3.1、§3.4 已实施并随 V1 关闭；生命周期、平台与规模矩阵由本文件 §1 承接。
...
压测与回归矩阵、ASAN 补充诊断）与 §6（生产配置基线：Gene INI、宿主 OPcache、模板编译配置）。
执行新优化前先建立该条目所需基线；V1 移交项按本文件 §1 独立推进，不反向阻塞 V1 关闭。
