...
> 版本：v4（2026-09-06）。v4 在 v3 基础上补方法论门禁与优先级重排，**不新增性能条目**：
> ① 新增 §0.1 自上而下 profiling 前置门禁与 §0.2 期望值校准（v3 全部条目为 bottom-up 源码复核所得，
>    缺少「该条目是否真在热路径」的证据）；② 拆出 §2.1b（`view_compile_check_mtime` 默认值缺陷，
>    经核对 `gene.c:153-154` 两个开关默认均为 0，为本文档最高 ROI 项）；
> ③ 拆出 §3.1b（冻结表 tombstone 耗尽 → 静默拒写，定性为功能缺陷，不再挂在 §3.1 拆表之后）；
> ④ 新增 §7.2 宿主 PHP 配置基线（OPcache/realpath/JIT/preload），并将其设为全部 A/B 的固定前提；
> ⑤ §8 重排批次、§9.0 明确「先 profile 再微基准」的顺序纪律。
>
...
> 2. **自上而下优先（P0 门禁，见 §0.1）**。本文档 §1–§6 是逐文件静态复核攒出的候选清单，
>    条目顺序不代表热度。任何条目进入性能批次前，必须先在真实应用 profile 中命中热点。
> 3. 涉及生命周期/裸指针的改动，Linux `-fsanitize=address` 下跑全量回归（见 §9）。
> 4. 任何改变**公开 API 语义**的项目单独立项、单独评审，不得混入「透明性能优化」批次。

---

## 0. 前置门禁、期望值校准与已有基线

### 0.1 【P0 前置门禁】自上而下 profiling，先定位再优化

本文档最大的方法论风险：整份清单是**读源码攒出来的**（bottom-up），没有任何一步证明这些条目
落在真实请求的 CPU 时间里。按现状执行，很可能对占请求总时间 0.1% 的分支做了严谨的微基准，
然后合入一堆通不过自己 §9.2 准入标准的改动。

**因此在 §8 第一批开始之前，必须先产出下述证据，缺失即不得进入任何性能改造条目**：

1. **代表性负载定义**：至少两条 —— ① 纯路由/渲染（无 DB）；② 含 DB + ORM + 模板的典型业务请求。
   两者都在生产配置下运行（`run_environment >= 2`、§2.1 正确的视图配置、宿主 PHP 按 §7.2 调优）。
2. **CPU flamegraph**：Linux Swoole worker 用 `perf record -F 999 -g -p <worker_pid>` +
   ...
   同时记录 `gene.so` 自身符号占总 on-CPU 时间的比例 —— **若 `gene.so` 整体不足 10%，
   本文档 §1/§4/§5/§6 的所有微优化上限就是那 10% 的一个零头，应直接转入 §7.2 宿主调优与
   §2.1/§4.9 配置类问题，性能批次全部搁置。**
3. **分配器计数**：对 §1.4（emalloc 次数）、§1.2/§4.3/§4.4（哈希探测次数）这类「省次数」型条目，
   用计数器或 `ltrace`/`valgrind --tool=callgrind` 直接给出每请求实际次数，而不是从源码推断。
4. **热点映射表**：把 flamegraph top-20 帧逐条映射到本文档条目编号；**未被映射到的条目降级为
   「维护性改动」**，可为代码整洁合入，但不得记入性能收益、不占性能批次名额、不要求跑 §9 全套基准。

### 0.2 已知的期望值校准

- 第一批四项（§1.2 省 1 次哈希探测、§1.7 常量 `strlen`（文档自身已承认编译器多半已折叠）、
  §4.4 known-function、§5.7 Benchmark C API）在需要走 DB 与模板渲染的请求中，
  合计**大概率处于测量噪声量级**。按 §9.2「无显著收益则不合入性能改动」，它们很可能
  全部通不过自己的门禁。**定位调整**：默认视为维护性/代码质量改动；只有 §0.1 第 4 步
  的热点映射把它们打进 top-20，才升格为性能条目。
...
### 第零批：配置与正确性 —— 不受 profiling 门禁与收益门禁约束，**最先落地**
1. §2.1 修正文档/部署检查；§7.2 落地宿主 PHP 基线（OPcache/realpath），作为后续全部 A/B 的固定前提。
2. §2.1b `view_compile_check_mtime` 默认值缺陷立项（本文档最高 ROI 项，纯亏损配置，无需基准）。
3. §3.1b 候选 3 的观测部分：导出 `nNumUsed/nNumOfElements/nTableSize` 与
   `cache_insert_refused`，让 tombstone 耗尽可被监控发现（低风险、无语义变更）。
4. §1.1 / §3.1 补 route_pc、clear、借用指针与 churn 最小复现；已复现的内存安全或数据正确性
   问题作为修复单独处理，不绑在拆表或默认开关调整上。
5. §4.9 部署检查：生产必须 `gene.run_environment >= 1`。

### 第零批之二：profiling 门禁（**性能批次的准入前提**）
1. §0.1 产出两条代表性负载的 flamegraph，给出 `gene.so` 占 on-CPU 时间的比例。
2. §0.1 第 4 步热点映射表：把 top-20 帧映射到条目编号；未命中者降级为维护性改动。
3. §9 复用现有验收入口，仅为**已命中热点**的条目补微基准，保存版本与原始结果。
4. **若 `gene.so` 整体占比不足 10%**：性能批次整体搁置，工作转入 §7.2 宿主调优、
   §2.1b/§4.9 配置类问题与第零批的正确性修复。

### 第一批：低风险候选 —— **默认定位为维护性改动**（见 §0.2）
以下四项合计大概率处于测量噪声量级；**仅当 §0.1 热点映射将其打进 top-20 时才升格为性能条目
并要求跑 §9 全套基准**，否则按代码质量改动合入，不记性能收益、不占性能批次名额。

| 项目 | 准入条件与验收 |
|---|---|
| §1.2 action 单次查找 | 保留方法可见性、未命中与异常行为，不跨 dispatch 缓存 |
| §1.7 字符串长度/Json 函数缓存 | 排除 DI 展平和内部 JSON API 直调；证实确有运行时工作可省（编译器多半已折叠常量 `strlen`）|
...
### 架构/API 专项：不混入透明优化批次
- §3.1b 候选 1（tombstone 复用）在第零批观测数据确认问题真实发生后单独评审；
...
### 9.0 顺序纪律：先 profile，再微基准
微基准只能回答「这个改动让这段代码快了多少」，**不能**回答「这段代码值不值得改」。
后者只有 §0.1 的 flamegraph 能回答。因此本节全部手段的执行顺序是：
**§0.1 profiling → 热点映射 → 仅对命中条目建微基准 → §9.4 端到端压测复核**。
反向执行（先为清单里每一项建微基准）会产出大量「确实变快了但请求没变快」的改动。
