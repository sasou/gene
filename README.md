...
> 💡 **Gene 6.2.5** — V3 代码级优化全量落地（NTS 锁编译期消除、路由/DI/Db 热路径去分配、冻结框架表零拷贝、ctx 冷热分离、双池 C 层 idle 栈）+ 第二轮审计修复收敛；新增 `view_stat_ttl`/`view_fresh_max`/`log_keep_open`/`log_reopen_interval` 四项 INI 与 Monitor 可观测字段；Linux + 真实 Swoole 主门禁 17/17 PASS。
