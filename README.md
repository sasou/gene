...
[![Version](https://img.shields.io/badge/version-6.1.0-blue.svg)](https://github.com/sasou/php-gene)
...
- 📦 **全功能栈**：路由、缓存、依赖注入、数据库连接池、钩子（路由级中间件管道为后续提案）等完整组件
...
| **数据库** | PDO 四驱动 + ORM v2（有序 Query、批量写、行锁、IN） | ⭐⭐⭐⭐⭐ |
| **缓存系统** | 方法级缓存，实时版本缓存，多种后端支持 | ⭐⭐⭐⭐⭐ |
| **出站 HTTP** | `Gene\Http`（FPM=curl，Swoole=协程客户端）；同进程互调 `Invoke`/`Rest` | ⭐⭐⭐⭐☆ |
| **请求袋 / 原语** | `Context`、`Request::json`、`Json`、SSE、`Crypto`、限流/锁 | ⭐⭐⭐⭐☆ |
| **视图引擎** | 编译模板，原生PHP模板，布局支持 | ⭐⭐⭐⭐☆ |
| **钩子** | AOP面向切面风格的路由钩子，配置注册，解耦调用（路由级中间件管道为后续提案） | ⭐⭐⭐⭐⭐ |
| **会话管理** | 多驱动支持，Swoole适配 | ⭐⭐⭐⭐☆ |
| **国际化** | 多语言方案，灵活配置 | ⭐⭐⭐⭐☆ |
| **命令行** | 控制台程序，守护进程支持 | ⭐⭐⭐⭐☆ |  
