...
5. **M4：公共字符串工具边界防护**
   - `str_init`、`str_sub_len`、`str_append` 增加 NULL 入参检查。
   - `str_sub_len` 校验 `start` 是否越界，并将过长的 `len` 收敛到源字符串剩余长度，避免越界读取。
   - `str_append` 改用已知长度的 `memcpy` 追加，避免重复扫描。

6. **L1：数据库连接池命名缓存清理**
   - 复核确认连接池已实现 `GENE_MSHUTDOWN_FUNCTION(pool)`，并已由主模块 MSHUTDOWN 调用；异常关停时会清理 `gene_pool_named_cache`，该问题在当前源码中已实际解决。

7. **L2：PDO 错误属性读取临时值释放**
   - 初始化 `zend_read_property` 的 `rv` 输出槽，并在正常返回与命中错误关键字的提前返回路径统一释放可能产生的临时值。

8. **L3：Memcached 配置错误路径释放**
   - Memcached 与 Windows Memcache 两套初始化流程在 `servers` 缺失或类型错误时，先释放已创建对象，再显式返回失败，避免依赖 E_ERROR bailout 回收。

9. **L4：路由路径解析不再修改输入**
   - 路由递归匹配直接使用“指针 + 长度”查询路径片段，移除临时写入 `\0` 及恢复 `/` 的逻辑，可安全处理只读字符串。

10. **L5：公共插入与替换辅助函数所有权**
    - `insertAll` 移除会被覆盖且没有实际语义的 `dest` 入参，改为明确返回新分配缓冲区。
    - `replace_string` 改为只读扫描输入并直接构造结果，不再通过 `php_strtok_r` 破坏源字符串，也不存在失败后无法恢复输入的问题。

11. **L6：路由初始化路径所有权**
    - `get_path_router_init` 统一返回新分配字符串，包括空路径和无需变换的路径；调用方始终接管并释放返回值，不再通过指针比较推断所有权。

### 尚未修复

- 无。本报告确认的 H1、M1-M4、L1-L6 均已落地或复核确认为当前源码已具备对应修复。

## 四、验证结果

- `git diff --check`：通过。
- Windows 当前未提供 PHP 扩展编译工具链，无法重新编译本次 C 源码；本机 PHP 8.1.34 NTS 已加载的是修改前构建的 Gene 扩展。
- 使用现有扩展运行 `RouterTest.php`：35 通过、0 失败；测试产生的缺失模板、控制器和测试文件警告属于既有用例预期。
- 使用现有扩展运行 `CacheTest.php`：22 通过、2 失败；失败项为版本缓存/清理断言，与本次源码改动无直接关联。
- 使用现有扩展运行 `DatabaseTest.php`：1 通过、3 失败；失败原因为本地 MySQL 凭据、PostgreSQL 服务不可用及现有 SQLite 测试调用不存在的 `connect()` 方法。
- 建议在 Linux 环境执行 `phpize && ./configure && make`，并分别完成非 ZTS 与 ZTS 构建和测试，以验证本次 C 层改动。
...
本轮已完成 H1、M1-M4、L1-L6 全部确认问题的修复或复核闭环。受当前 Windows 工具链限制，最终合入前仍应补充实际编译及 ZTS/非 ZTS 双构建验证。
